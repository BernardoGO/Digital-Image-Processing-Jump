...
imgLines = imgLines(Rect(500,550,300,170));
...
Canny(imgLines, imgLines, 50, 200, 3);
vector<Vec4i> lines;
  HoughLinesP(imgLines, lines, 0.8, CV_PI/360, 50, 50, 40 );
...
for( size_t i = 0; i < lines.size(); i++ )
  {
    ...
    double dfd = sqrt(pow(Point(l[0], l[1]).x - Point(l[2], l[3]).x, 2) + pow(Point(l[0], l[1]).y - Point(l[2], l[3]).y, 2));
        if(dfd > 60 && dfd < 110)
    //cout << dfd;
    line( imgOriginal, Point(l[0]+500, l[1]+550), Point(l[2]+500, l[3]+550), Scalar(0,0,255), 3, CV_AA);
  }
